...
{
	sealed public class TFTP : ServerProvider, ITFTPServer_Provider, IDisposable
	{
		TFTPSocket socket;
		...
		public TFTP(IPEndPoint endpoint)
		{
			...
			this.socket.DataReceived += DataReceived;
			this.socket.DataSend += DataSend;
		}
				...
				switch (request.OPCode)
				{
					...
					case (int)TFTPOPCodes.ERR:
						if (request.ErrorCode != 0)
							Handle_Error_Request(request.ErrorCode, request.ErrorMessage, true);
						else
						{
							if (Clients.ContainsKey(this.remoteEndp.Address))
					...
					case (int)TFTPOPCodes.ACK:
						...
						break;
					default:
						Handle_Error_Request(TFTPErrorCode.IllegalOperation, "Unknown OPCode: {0}".F(request.OPCode));
						break;
				}
			}
		}
		...
		void Send(ref TFTPPacket packet, IPEndPoint endpoint)
		{
			if (Clients.ContainsKey(this.remoteEndp.Address))
		...
		public void Handle_RRQ_Request(object packet)
		{
			...
			{
				Handle_Error_Request(TFTPErrorCode.FileNotFound, Clients[this.remoteEndp.Address].FileName);
				return;
			}
			else
			{
				...
				{
					Handle_Option_request(Clients[this.remoteEndp.Address].TransferSize, Clients[this.remoteEndp.Address].BlockSize);
					return;
				}
		...
		public void SetMode(TFTPMode mode)
		{
			if (mode != TFTPMode.Octet)
				Handle_Error_Request(TFTPErrorCode.InvalidOption, "Invalid Option");
			else
			{
				if (Clients.ContainsKey(this.remoteEndp.Address))
		...
		internal void Handle_Option_request(long tsize, int blksize)
		{
			...
			Send(ref packet, Clients[this.remoteEndp.Address].EndPoint);
		}
		...
		public void Handle_Error_Request(TFTPErrorCode error, string message, bool clientError = false)
		{
			...
			if (!clientError)
			{
				...
				Send(ref response, Clients[this.remoteEndp.Address].EndPoint);
			}
