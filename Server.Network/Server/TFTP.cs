	...
	public sealed class TFTP : ServerProvider, ITFTPServer_Provider, IDisposable
	{
		...
		public void Handle_RRQ_Request(object request)
		{
			lock (Clients)
			{
				...
				{
					this.Handle_ERR_Request(TFTPErrorCode.AccessViolation, 
				...
				if (Filesystem.Exist(file) && !string.IsNullOrEmpty(file))
				{
					...
					this.Handle_ERR_Request(TFTPErrorCode.FileNotFound, file, packet.Source);
			}
		}
		...
		internal override void DataReceived(object sender, DataReceivedEventArgs e)
		{
			...
			switch (request.OPCode)
			{
				...
				case TFTPOPCodes.ERR:
					this.Handle_ERR_Request(request.ErrorCode, request.ErrorMessage, request.Source, true);
					break;
				case TFTPOPCodes.ACK:
					...
					break;
				default:
					this.Handle_ERR_Request(TFTPErrorCode.IllegalOperation, "Unknown TFTP OPCode: {0}".F(request.OPCode), request.Source);
					break;
			}
		}
		...
		internal void Handle_Option_request(long tsize, ushort blksize, int winsize, ushort mswinsize, IPEndPoint client)
		{
			lock (Clients)
			{
				...
				offset += CopyTo(ref blksizeopt, 0, ref tmpbuffer, offset, blksizeopt.Length) + 1;

				var blksize_value = Exts.StringToByte(blksize.ToString(), Encoding.ASCII);
				offset += CopyTo(ref blksize_value, 0, ref tmpbuffer, offset, blksize_value.Length) + 1;

				var tsizeOpt = Exts.StringToByte("tsize", Encoding.ASCII);
				offset += CopyTo(ref tsizeOpt, 0, ref tmpbuffer, offset, tsizeOpt.Length) + 1;

				var tsize_value = Exts.StringToByte(tsize.ToString(), Encoding.ASCII);
				offset += CopyTo(ref tsize_value, 0, ref tmpbuffer, offset, tsize_value.Length) + 1;

				if (winsize > 1)
				{
					var winOpt = Exts.StringToByte("windowsize", Encoding.ASCII);
					offset += CopyTo(ref winOpt, 0, ref tmpbuffer, offset, winOpt.Length) + 1;

					var winsize_value = Exts.StringToByte(winsize.ToString(), Encoding.ASCII);
					offset += CopyTo(ref winsize_value, 0, ref tmpbuffer, offset, winsize_value.Length) + 1;

					if (Settings.AllowVariableWindowSize)
					{
						var mswinOpt = Exts.StringToByte("msftwindow", Encoding.ASCII);
						offset += CopyTo(ref mswinOpt, 0, ref tmpbuffer, offset, mswinOpt.Length) + 1;

						var mswinsize_value = Exts.StringToByte(mswinsize.ToString(), Encoding.ASCII);
						offset += CopyTo(ref mswinsize_value, 0, ref tmpbuffer, offset, mswinsize_value.Length) + 1;
					}
				}
		...
		internal void Readfile(IPEndPoint client)
		{
			lock (Clients)
			{
				...
				{
					this.Handle_ERR_Request(TFTPErrorCode.AccessViolation, ex.Message, client);
				}
			}
		}
	}
